...
C-----------------------------------------------------------------------
      STATEV_PAR=0.D0 ! 0: STATEV FROM SIMULATION | 1: READ STATEV FROM CSV FILE
      PLOT_PAR=0.D0 ! 0: PI-PLANE + CMAP | 1: PI-PLANE
      FILEPROPS='PROPS_EDDQ_HAH20.CSV'
      FILESTATV='STATEV_EDDQ_HAH20.CSV'
      MAX_ITER=200
C-----------------------------------------------------------------------
...
C
      ...
      ALLOCATE(DDFDDS(NTENS,NTENS), T(NTENS,NTENS))
C
C-----------------------------------------------------------------------
...
C     1.4     YIELD SURFACE PLOTTING
      ...
      OPEN(103, FILE='OUT\DFDS.CSV', STATUS='UNKNOWN')
      YL_SIG=0.D0
      PI_SIG=0.D0
      DO I=0, 360
          ANG=I*RADIAN
...
C       [2] ORIGINAL YIELD SURFACE
          ...
          WRITE(101, 2) DUMMY11(1), DUMMY11(2), DUMMY12(1), DUMMY12(2)
      END DO
      ANG=0.D0*RADIAN
      STRESS=0.D0
      STRESS(1)= DCOS(ANG)
      STRESS(2)= DSIN(ANG)
      IF(HARD_PAR .GE. 4.) THEN
          CALL DEVIATORIC_TENS(T)
          STRESS=MATMUL(T,STRESS)
      ...
      CALL PI_PLANE(STRESS, YL_SIG)
      CALL PI_PLANE3(DFDS, DFDS0)
      WRITE(102, 2) PI_SIG(1), PI_SIG(2), YL_SIG(1), YL_SIG(2)
      WRITE(103, 22) DFDS0(1), DFDS0(2)
      CLOSE(100)
      CLOSE(101)
      CLOSE(102)
      CLOSE(103)
...
C     # OBTAIN CONVERGENCE BEHAVIOR
          ...
          STRESS=STRESS/SIG_BAR
          CALL PI_PLANE(STRESS, PI_SIG)
          STRESS=STRESS*FLOW_SIG*(1.D0+14.D0)
...
C-----------------------------------------------------------------------
          ...
          EQPLAS0=EQPLAS
          DFDS=0.D0
          CALL STRESS_UPDATE(HARD_VAR0,STRESS,EQPLAS0,SIG_BAR0,
     1      DDSDDE0,ITER)
          IF(HARD_PAR.EQ.1. .OR. HARD_PAR.EQ.2.) THEN
            DO K=1, NDIM3
              STRESS(K)=STRESS(K)+HARD_VAR(K)
