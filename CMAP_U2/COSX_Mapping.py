...
cosx_data = np.loadtxt('OUT\COSX.csv', delimiter=',', dtype=np.float64)
...
cmap_cosx=np.zeros([ang1,ang2,3])
for i in range(ang1):
      for j in range(ang2):
            if cmap_data[indx,2] == 0:
                  ...
                  nconv=nconv+1
            cmap_cosx[i,j,0]=cosx_data[indx,0]
            cmap_cosx[i,j,1]=cosx_data[indx,1]
            cmap_cosx[i,j,2]=cosx_data[indx,2]
            indx=indx+1
#########################################################################################
...
plt.plot([0,0],[pp_axis[2,1],pp_axis[1,1]], 'k', linewidth=3)
r2=r*1.02
...
#plt.text(0.2, 1.0, r'Mean iter: '+str(int(mean_iter)),size=25, bbox=props)
#props2=dict(boxstyle='square', facecolor='white', alpha=0.5)
#plt.text(-1.5, 1.0, r'(d)',size=30, bbox=props2)
plt.plot(stress[:,0], stress[:,1], 'k', linewidth=2.5, label='HAH'+'$_{20}$')
plt.plot(stress[:,2], stress[:,3], 'k--', linewidth=2.5, label='Isotropic hardening')
plt.legend(loc=4, bbox_to_anchor=(1.1,0), fontsize='xx-large')
#plt.plot(stress2[:,0], stress2[:,1], 'k', linewidth=2.5)

step=10
cmap = plt.get_cmap('RdBu',step)
#cmap = plt.get_cmap('jet',step)
plt.scatter(cmap_cosx[:,:,0], cmap_cosx[:,:,1], c=cmap_cosx[:,:,2], cmap=cmap, s=2, vmin=-1, vmax=1)

cb=plt.colorbar()
cb.ax.set_yticklabels(labels=np.linspace(-1,1,step+1), size=25)
