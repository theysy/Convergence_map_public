...
#max_iter=20 # CPPM
#max_iter=40 # LSEBM
cmap = plt.get_cmap('Blues',max_iter)
plt.scatter(cmap_output[:,:,0], cmap_output[:,:,1], c=cmap_output[:,:,2], cmap=cmap, s=2, vmin=0, vmax=max_iter)

cb=plt.colorbar()
label=np.arange(0,max_iter,1/step)
label2=np.linspace(0,max_iter,step)
cb.ax.set_yticklabels(labels=label, size=25)
#cb.ax.set_yticklabels(labels=range(max_iter), size=25)
